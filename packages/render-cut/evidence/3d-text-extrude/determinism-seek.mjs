...
// packages/render-cut/evidence/3d-text-flat/determinism-seek.mjs を踏襲した（extrude 用に移植）。
//
// シーン: carousel（テキストブロック全体が Y 軸回転）の「立体テロップ」extrude
// （このシーンは golden.mjs の「回転中の側面厚み」「裏面鏡文字」証跡にも使う）。
...
import { carouselScene } from "./support/scenes.mjs";
...
async function runDeterminism() {
  ...
  for (const run of [1, 2]) {
    ...
    const overlay = overlayFor("title", carouselScene(), { start: 0, duration: DURATION });
...
const result = {
  generated_at_note: "generated by evidence/3d-text-extrude/determinism-seek.mjs",
  scene: "carousel (texts[] extrude, id=title, 立体テロップ)",
  output: edit.output,
