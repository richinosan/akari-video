...
test('defaultRootPath: posix は HOME 起点で <home>/AkariVideo を返す', () => {
    const result = defaultRootPath({ HOME: '/Users/example' }, { platform: 'darwin' });
    assert.equal(result, join('/Users/example', 'AkariVideo'));
});
...
test('defaultRootPath: win32 は USERPROFILE 起点になる（実 Windows 不要・env 注入で分岐を検証）', () => {
    ...
    assert.equal(result, join('C:\\Users\\example', 'AkariVideo'));
});
...
test('defaultRootPath: win32 でも USERPROFILE が優先される（HOMEDRIVE/HOMEPATH より優先）', () => {
    ...
    assert.equal(result, join('C:\\Users\\primary', 'AkariVideo'));
});
